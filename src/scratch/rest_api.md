...
To control the PORT, POST trigger messages:
    ...
    /:zoneID/server/:serverID/port/:portID/trigger/:trigger(/:offset?)
...
To force a hard jump to specific point, ideally when stopped and not on air, POST to:
    ...
    /:zoneID/server/:serverID/port/:portID/jump/:offset
...
1. Prepare the jump by PUTing an empty document to `/:zoneID/server/:serverID/port/:portID/jump/:offset`
...
To query the currently set jump point, send a GET request to the same URL. The response is a JSON document similar to the following example:

```JSON
{
  "type": "JumpPoint",
  "serverID": 1001,
  "portName": "mixer-port-7",
  "offset": 425
}
```
